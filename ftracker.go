...
func distance(action int) float64 { // Результат в км
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	callories := runningCaloriesMeanSpeedMultiplier * meanSpeed * runningCaloriesMeanSpeedShift * weight / mInKm * duration * minInH
	return callories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	meanSpeed := meanSpeed(action, duration)
	callories := (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeed*kmhInMsec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return callories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	callories := (meanSpeed + swimmingCaloriesMeanSpeedShift) * 2 * weight * duration
	return callories
}
